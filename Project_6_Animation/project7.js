...
function SimTimeStep( dt, positions, velocities, springs, stiffness, damping, particleMass, gravity, restitution )
{
    ...
    var totalForces = new Array(positions.length);
    var gravityForce = gravity.mul(particleMass);
    
    // gravitational force
    for(let particleIdx = 0; particleIdx < totalForces.length; particleIdx++) {
        totalForces[particleIdx] = gravityForce.copy();
    }

    for(let springIdx = 0; springIdx < springs.length; springIdx++) {
        const currentSpring = springs[springIdx];
        const particleA = currentSpring.p0;
        const particleB = currentSpring.p1;
        
        // current state vectors
        const posA = positions[particleA].copy();
        const posB = positions[particleB].copy();
        const velA = velocities[particleA].copy();
        const velB = velocities[particleB].copy();
        
        const displacement = posB.sub(posA);
        const currentLength = displacement.len();
        const normalizedDirection = displacement.div(currentLength);
        
        // Spring force
        const lengthDifference = currentLength - currentSpring.rest;
        const elasticForce = normalizedDirection.mul(stiffness * lengthDifference);
        
        // Damping force 
        const relativeVelocity = velB.sub(velA);
        const velocityProjection = relativeVelocity.dot(normalizedDirection);
        const dampingForce = normalizedDirection.mul(damping * velocityProjection);
        
        // Equal and opposite forces
        const combinedForce = elasticForce.add(dampingForce);
        totalForces[particleA].inc(combinedForce);
        totalForces[particleB].dec(combinedForce);
    }
        
	// [TO-DO] Update positions and velocities
    for(let particleIdx = 0; particleIdx < positions.length; particleIdx++) {
        // a = F/m
        const acceleration = totalForces[particleIdx].div(particleMass);
        // Update vel
        velocities[particleIdx].inc(acceleration.mul(dt));
        // Update pos
        positions[particleIdx].inc(velocities[particleIdx].mul(dt));
    }
    
    // [TO-DO] Handle collisions
    const boundaryLimits = { min: -1.0, max: 1.0 };
    const axisLabels = ['x', 'y', 'z'];
    
    for(let particleIdx = 0; particleIdx < positions.length; particleIdx++) {
        const currentPos = positions[particleIdx];
        const currentVel = velocities[particleIdx];
        
        // check for boundary violations
        axisLabels.forEach(axis => {
            const coordinate = currentPos[axis];
            // lower boundary collision
            if(coordinate < boundaryLimits.min) {
                const penetrationDepth = boundaryLimits.min - coordinate;
                currentPos[axis] = boundaryLimits.min + (penetrationDepth * restitution);
                currentVel[axis] = -currentVel[axis] * restitution;
            }
            // upper boundary collision  
            else if(coordinate > boundaryLimits.max) {
                const penetrationDepth = coordinate - boundaryLimits.max;
                currentPos[axis] = boundaryLimits.max - (penetrationDepth * restitution);
                currentVel[axis] = -currentVel[axis] * restitution;
            }
        });
    }
